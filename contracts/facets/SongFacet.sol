...
contract SongFacet {
    ...
    function uploadAndMintSong(
        ...
        uint256 _albumId // 0 if no album
    ...
    function updateSongMetaData(uint256 songId, string memory newCid) external {
        LibAppStorage.AppStorage storage aps = LibAppStorage.appStorage();
    ...
    function getAllSongs() external view returns (uint256[] memory) {
        return LibAppStorage.appStorage().allSongIds;
    }

    function getSongById(uint256 songId) external view returns (LibAppStorage.Song memory) {
        return LibAppStorage.appStorage().songIdToSong[songId];
    }
    ...
    function _mintSongNFT(address artist, string memory cid, LibERC721Storage.ERC721Storage storage es)
        private
